...
function LogPanel({ onToggleAll, onSetLogs, logs }) {
  ...
  function handleStateToggle(){
    ...
    setStatus(!status)
    const logData = {
      type: status ? "warn" : "notify",
      msg: `${status ? "Activating" : "Decommissioning"} all hosts`
    }
    onSetLogs(logData)
  }
  ...
  return (
    <Segment className="HQComps" id="logPanel">
      ...
      <Button fluid color={status ? "red" : "green"} content={status ? "ACTIVATE ALL" : "Decommission All"} onClick={handleStateToggle}/>
    </Segment>
  );
}
